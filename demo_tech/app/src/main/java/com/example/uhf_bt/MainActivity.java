...
import com.example.uhf_bt.fragment.BleParamFragment;
import com.example.uhf_bt.fragment.RxWifiTestFragment;
...
import com.rscja.deviceapi.RFIDWithUHFRxNetwork;
import com.rscja.deviceapi.entity.UHFTAGInfo;
import com.rscja.deviceapi.interfaces.ConnectionStatus;
import com.rscja.deviceapi.interfaces.ConnectionStatusCallback;
import com.rscja.deviceapi.interfaces.IUHFRx;
...
public class MainActivity extends BaseActivity implements View.OnClickListener {
    ...
    private static final int REQUEST_SELECT_WIFI_DEVICE = 3;
    ...
    private Button btn_connect_bt; // 蓝牙模式的连接按钮
    private Button btn_search_wifi; // WiFi模式的搜索按钮
    private TextView tvAddress;
    public BluetoothAdapter mBtAdapter = null;
    public IUHFRx uhf;
    private ConnectionMode connectionMode = ConnectionMode.BLUETOOTH;
    ...
    public int parseWay = 0;
    public final static int PARSE_BY_HEX = 0;
    public final static int PARSE_BY_ASCII = 1;
    public final static int PARSE_BY_DECIMAL = 2;
    public boolean filterGibberish = false;
    private boolean isRestoringRadioButton = false;
    ...
    public boolean secretCodeFlag = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initUHFInstance(connectionMode);
    ...
    protected void onResume() {
        ...
        secretCodeFlag = getIntent().getBooleanExtra("SecretCodeFlag", false);
    ...
    protected void initUI() {
        ...
        btn_connect_bt.setOnClickListener(this);
        btn_search = (Button) findViewById(R.id.btn_search);
        btn_search.setOnClickListener(this);
        btn_search_wifi = (Button) findViewById(R.id.btn_search_wifi);
        btn_search_wifi.setOnClickListener(this);

        RadioGroup rgConnectionMode = findViewById(R.id.rgConnectionMode);
        RadioButton rbBluetooth = findViewById(R.id.rbBluetooth);
        RadioButton rbWifi = findViewById(R.id.rbWifi);
        LinearLayout llWifiParams = findViewById(R.id.llWifiParams);
        LinearLayout llBluetoothButtons = findViewById(R.id.llBluetoothButtons);
        etIP = findViewById(R.id.etIP);
        etPort = findViewById(R.id.etPort);

        String savedMode = SPUtils.getInstance(this).getSPString("CONNECTION_MODE", "BLUETOOTH");
        if ("WIFI".equals(savedMode)) {
            rbWifi.setChecked(true);
            connectionMode = ConnectionMode.WIFI;
            llWifiParams.setVisibility(View.VISIBLE);
            llBluetoothButtons.setVisibility(View.GONE);
        } else {
            llBluetoothButtons.setVisibility(View.VISIBLE);
        }

        String savedIP = SPUtils.getInstance(this).getSPString("WIFI_IP", "192.168.96.126");
        String savedPort = SPUtils.getInstance(this).getSPString("WIFI_PORT", "8080");
        etIP.setText(savedIP);
        etPort.setText(savedPort);

        rgConnectionMode.setOnCheckedChangeListener((group, checkedId) -> {
            if (isRestoringRadioButton) {
                return;
            }
            ...
            if (uhf != null && uhf.getConnectStatus() == ConnectionStatus.CONNECTED) {
                new AlertDialog.Builder(this)
                        ...
                        .setMessage(R.string.dialog_msg_switch_connection)
                        .setPositiveButton(R.string.dialog_btn_ok, (dialog, which) -> {
                            disconnect(true);
                            switchConnectionMode(checkedId, llWifiParams, llBluetoothButtons);
                        })
                        .setNegativeButton(R.string.dialog_btn_cancel, (dialog, which) -> {
                            isRestoringRadioButton = true;
                            ...
                            // 重置标志位
                            isRestoringRadioButton = false;
                        })
                        .setOnCancelListener(dialog -> {
                            // 点击外部或返回键时，也要恢复RadioButton状态
                            isRestoringRadioButton = true;
                            if (connectionMode == ConnectionMode.BLUETOOTH) {
                                rbBluetooth.setChecked(true);
                            } else {
                        ...
                        .show();
            } else if (uhf != null && uhf.getConnectStatus() == ConnectionStatus.CONNECTING) {
                isRestoringRadioButton = true;
                if (connectionMode == ConnectionMode.BLUETOOTH) {
                    rbBluetooth.setChecked(true);
                } else {
        ...
        mTabHost.addTab(mTabHost.newTabSpec(getString(R.string.title_inventory))
                .setIndicator(getString(R.string.title_inventory)), UHFReadTagFragment.class, null);
        ...
        mTabHost.addTab(mTabHost.newTabSpec(getString(R.string.uhf_msg_tab_set))
                .setIndicator(getString(R.string.uhf_msg_tab_set)), UHFSetFragment.class, null);

        mTabHost.addTab(mTabHost.newTabSpec(getResources().getString(R.string.uhf_radar_loaction)).setIndicator(
                getResources().getString(R.string.uhf_radar_loaction)), UHFRadarLocationFragment.class, null);
        ...
        mTabHost.addTab(
                mTabHost.newTabSpec(getString(R.string.title_2d_Scan)).setIndicator(getString(R.string.title_2d_Scan)),
                BarcodeFragment.class, null);

        mTabHost.addTab(mTabHost.newTabSpec(getString(R.string.uhf_msg_tab_read_write))
                .setIndicator(getString(R.string.uhf_msg_tab_read_write)), UHFReadWriteFragment.class, null);

        mTabHost.addTab(mTabHost.newTabSpec(getString(R.string.uhf_msg_tab_lock))
                .setIndicator(getString(R.string.uhf_msg_tab_lock)), UHFLockFragment.class, null);

        mTabHost.addTab(mTabHost.newTabSpec(getString(R.string.uhf_msg_tab_kill))
                .setIndicator(getString(R.string.uhf_msg_tab_kill)), UHFKillFragment.class, null);
        ...
        mTabHost.addTab(
                mTabHost.newTabSpec(getString(R.string.title_update)).setIndicator(getString(R.string.title_update)),
                UHFUpdataFragment.class, null);

        mTabHost.addTab(mTabHost.newTabSpec(getString(R.string.ble_param_title))
                .setIndicator(getString(R.string.ble_param_title)), BleParamFragment.class, null);

        mTabHost.addTab(mTabHost.newTabSpec(getString(R.string.title_bt_rename))
                .setIndicator(getString(R.string.title_bt_rename)), BTRenameFragment.class, null);

        mTabHost.addTab(
                mTabHost.newTabSpec(getString(R.string.title_bt_wifi)).setIndicator(getString(R.string.title_bt_wifi)),
                RxWifiTestFragment.class, null);

        // 根据连接模式显示/隐藏蓝牙相关的Tab
        updateBluetoothTabsVisibility();

        parseWay = SPUtils.getInstance(this).getSPInt(SPUtils.PARSE_WAY, 0);
        filterGibberish = parseWay == MainActivity.PARSE_BY_ASCII;
    }
            ...
            case R.id.btn_search_wifi:
                if (connectionMode == ConnectionMode.WIFI) {
                    if (uhf.getConnectStatus() == ConnectionStatus.CONNECTING) {
                        ...
                        Intent wifiIntent = new Intent(MainActivity.this, WifiDeviceListActivity.class);
                        startActivityForResult(wifiIntent, REQUEST_SELECT_WIFI_DEVICE);
                        cancelDisconnectTimer();
                    }
                }
                break;
    ...
    private void formatConnectButton(long disconnectTime) {
        Button connectBtn = getCurrentConnectButton();
        if (uhf.getConnectStatus() == ConnectionStatus.CONNECTED) {
            if (!isScanning && System.currentTimeMillis() - lastTouchTime > 1000 * 30 && timerTask != null) {
                long minute = disconnectTime / 1000 / 60;
                if (minute > 0) {
                    connectBtn.setText(getString(R.string.disConnectForMinute, minute)); // 倒计时分
                } else {
                    connectBtn.setText(getString(R.string.disConnectForSecond, disconnectTime / 1000)); // 倒计时秒
                }
            } else {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (!isScanning) {
            if (item.getItemId() == R.id.UHF_Battery) {
                ...
                if (connectionMode == ConnectionMode.BLUETOOTH && uhf instanceof RFIDWithUHFBLE) {
                    HashMap<String, String> versionMap = ((RFIDWithUHFBLE) uhf).getBluetoothVersion();
                    if (versionMap != null) {
                        String verMsg = getString(R.string.ble_param_firmware_version) + ": "
                                + versionMap.get(RFIDWithUHFBLE.VERSION_BT_FIRMWARE) + "\n"
                                + getString(R.string.ble_param_hardware_version) + ": "
                                + versionMap.get(RFIDWithUHFBLE.VERSION_BT_HARDWARE) + "\n"
                                + getString(R.string.ble_param_software_version) + ": "
                                + versionMap.get(RFIDWithUHFBLE.VERSION_BT_SOFTWARE);
                        Utils.alert(MainActivity.this, R.string.action_ble_ver, verMsg, R.drawable.webtext);
                    }
                } else {
                    showToast(R.string.msg_feature_bluetooth_only);
                }
                ...
                Utils.alert(this, R.string.disconnectTime, view, R.drawable.webtext,
                        new DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                int index = spDisconnectTime.getSelectedItemPosition();
                                long time = 1000 * 60 * 60 * index;
                                SPUtils.getInstance(getApplicationContext()).setSPInt(SPUtils.DISCONNECT_TIME_INDEX,
                                        index);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (requestCode) {
            case REQUEST_SELECT_DEVICE:
                ...
                if (resultCode == Activity.RESULT_OK && data != null) {
                    ...
                    if (data.getBooleanExtra("isSearch", true)) {
                        if (uhf instanceof RFIDWithUHFBLE) {
                            ((RFIDWithUHFBLE) uhf).startScanBTDevices((bluetoothDevice, i, bytes) -> {
                            });
                            SystemClock.sleep(1500);
                            ((RFIDWithUHFBLE) uhf).stopScanBTDevices();
                        }
                    }
            ...
            case REQUEST_ENABLE_BT:
                ...
                break;
            case REQUEST_SELECT_WIFI_DEVICE:
                if (resultCode == Activity.RESULT_OK && data != null) {
                    if (uhf.getConnectStatus() == ConnectionStatus.CONNECTED) {
                        disconnect(true);
                    }
                    String wifiIP = data.getStringExtra("WIFI_IP");
                    int wifiPort = data.getIntExtra("WIFI_PORT", 0);
                    if (!TextUtils.isEmpty(wifiIP) && wifiPort > 0) {
                        etIP.setText(wifiIP);
                        etPort.setText(String.valueOf(wifiPort));
                        // 自动连接
                        connectWifi();
                    }
                }
            ...
            case PERMISSION_REQUEST_EXTERNAL_STORAGE:
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                    if (!Environment.isExternalStorageManager()) {
                        showPermissionAlertDialog(getString(R.string.permission_external_storage),
                                (dialog, which) -> checkReadWritePermission());
                    }
                } else {
    ...
    public void connectBluetooth(String deviceAddress) {
        if (uhf.getConnectStatus() == ConnectionStatus.CONNECTING) {
            ...
            RFIDWithUHFBLE bleUhf = getUhfBLE();
            if (bleUhf != null) {
                bleUhf.connect(deviceAddress, btStatus);
            }
        }
    }
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (connectionStatus == ConnectionStatus.CONNECTED) {
                        ...
                        timeCountCur = SPUtils.getInstance(getApplicationContext()).getSPLong(SPUtils.DISCONNECT_TIME,
                                0);
                        if (timeCountCur > 0) {
                        ...
                        boolean reconnect = SPUtils.getInstance(getApplicationContext())
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        Log.v(TAG, "onRequestPermissionsResult requestCode=" + requestCode + " permissions="
        ...
        switch (requestCode) {
            ...
            case PERMISSION_REQUEST_ACCESS_FINE_LOCATION:
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    showPermissionAlertDialog(getString(R.string.permission_location),
                            (dialog, which) -> checkLocationPermission());
                }
                break;
            case PERMISSION_REQUEST_BLUETOOTH:
                if (grantResults.length == 0 || grantResults[0] == PackageManager.PERMISSION_DENIED) {
                    showPermissionAlertDialog(getString(R.string.permission_bluetooth),
                            (dialog, which) -> checkBluetoothPermission());
                } else {
                    checkReadWritePermission();
                }
                break;
                    ...
                    if (ndefMessage != null) {
                        for (NdefRecord record : ndefMessage.getRecords()) {
                            ...
                            if (record.toMimeType().equals("application/vnd.bluetooth.ep.oob")) {
                                ...
                                RFIDWithUHFBLE bleUhf = getUhfBLE();
                                if (bleUhf != null) {
                                    bleUhf.startScanBTDevices((bluetoothDevice, i, bytes) -> {
                                    });
                                    SystemClock.sleep(1000);
                                    bleUhf.stopScanBTDevices();
                                }
    ...
    private void initUHFInstance(ConnectionMode mode) {
        if (uhf != null) {
            uhf.free();
        }
        if (mode == ConnectionMode.BLUETOOTH) {
            ...
            uhf = RFIDWithUHFRxNetwork.getInstance();
        }
        uhf.init(getApplicationContext());
    }

    private void switchConnectionMode(int checkedId, LinearLayout llWifiParams, LinearLayout llBluetoothButtons) {
        if (checkedId == R.id.rbBluetooth) {
            connectionMode = ConnectionMode.BLUETOOTH;
            llWifiParams.setVisibility(View.GONE);
            llBluetoothButtons.setVisibility(View.VISIBLE);
            SPUtils.getInstance(this).setSPString("CONNECTION_MODE", "BLUETOOTH");
            initUHFInstance(ConnectionMode.BLUETOOTH);
            updateBluetoothTabsVisibility();
            // 切换模式后更新按钮状态
            formatConnectButton(0);
            mTabHost.setCurrentTab(0);
        } else if (checkedId == R.id.rbWifi) {
            connectionMode = ConnectionMode.WIFI;
            llWifiParams.setVisibility(View.VISIBLE);
            llBluetoothButtons.setVisibility(View.GONE);
            SPUtils.getInstance(this).setSPString("CONNECTION_MODE", "WIFI");
            initUHFInstance(ConnectionMode.WIFI);
            updateBluetoothTabsVisibility();
            // 切换模式后更新按钮状态
            formatConnectButton(0);
            mTabHost.setCurrentTab(0);
    ...
    private void connectWifi() {
        ...
        if (TextUtils.isEmpty(ip)) {
            showToast(R.string.msg_please_enter_ip);
            return;
        }
        if (TextUtils.isEmpty(portStr)) {
            showToast(R.string.msg_please_enter_port);
            return;
        }
        try {
            int port = Integer.parseInt(portStr);
            SPUtils.getInstance(this).setSPString("WIFI_IP", ip);
            SPUtils.getInstance(this).setSPString("WIFI_PORT", portStr);
            connectToWifi(ip, port);
        } catch (NumberFormatException e) {
            showToast(R.string.msg_port_format_error);
        }
    }

    private void connectToWifi(String ip, int port) {
        if (uhf.getConnectStatus() == ConnectionStatus.CONNECTING) {
            showToast(R.string.connecting);
            return;
        }
        showToast(R.string.connecting);
        if (uhf instanceof RFIDWithUHFRxNetwork) {
            ((RFIDWithUHFRxNetwork) uhf).setConnectionStatusCallback(new ConnectionStatusCallback() {
                @Override
                public void getStatus(ConnectionStatus connectionStatus, Object device) {
                    runOnUiThread(() -> {
                        if (connectionStatus == ConnectionStatus.CONNECTED) {
                            String ipAddr = (String) device;
                            tvAddress.setText(String.format("%s:%d\nconnected", ipAddr, port));
                            getCurrentConnectButton().setText(R.string.disConnect);
                            showToast(R.string.connect_success);
                        } else if (connectionStatus == ConnectionStatus.DISCONNECTED) {
                            isKeyDownUP = false;
                            getCurrentConnectButton().setText(R.string.Connect);
                            tvAddress.setText("disconnected");
                            showToast(R.string.disconnect);
                        }
                        for (IConnectStatus iConnectStatus : connectStatusList) {
            ...
            new Thread(() -> {
                ((RFIDWithUHFRxNetwork) uhf).connect(ip, port);
            }).start();
        }
    }

    public RFIDWithUHFBLE getUhfBLE() {
        if (connectionMode == ConnectionMode.BLUETOOTH && uhf instanceof RFIDWithUHFBLE) {
            return (RFIDWithUHFBLE) uhf;
        }
        return null;
    }
    ...
    public RFIDWithUHFRxNetwork getUhfNetwork() {
        if (connectionMode == ConnectionMode.WIFI && uhf instanceof RFIDWithUHFRxNetwork) {
            return (RFIDWithUHFRxNetwork) uhf;
        }
        return null;
    }

    public boolean isBluetoothMode() {
        return connectionMode == ConnectionMode.BLUETOOTH;
    }

    public boolean isWifiMode() {
        return connectionMode == ConnectionMode.WIFI;
    }

    /**
     * 获取当前模式的连接按钮
     */
    private Button getCurrentConnectButton() {
        return connectionMode == ConnectionMode.WIFI ? btn_connect : btn_connect_bt;
    }

    /**
     * 根据连接模式更新蓝牙相关Tab的显示/隐藏
     * BLE参数Tab索引：9
     * 修改蓝牙名称Tab索引：10
     */
    private void updateBluetoothTabsVisibility() {
        try {
            android.widget.TabWidget tabWidget = mTabHost.getTabWidget();
            boolean show = connectionMode == ConnectionMode.BLUETOOTH;

            // BLE参数Tab（索引9）
            if (tabWidget.getChildCount() > 9) {
                View bleParamTab = tabWidget.getChildAt(9);
                if (bleParamTab != null) {
                    bleParamTab.setVisibility(show ? View.VISIBLE : View.GONE);
                }
            }

            // 修改蓝牙名称Tab（索引10）
            if (tabWidget.getChildCount() > 10) {
                View btRenameTab = tabWidget.getChildAt(10);
                if (btRenameTab != null) {
                    btRenameTab.setVisibility(show ? View.VISIBLE : View.GONE);
                }
            }
        } catch (Exception e) {
            Log.e(TAG, "Error updating bluetooth tabs visibility", e);
        }
    }
     ...
     * 显示波特率设置和实时速率
     */
    public void enableSecretFeatures() {
        secretCodeFlag = true;
        // 通知所有Fragment更新UI
        FragmentManager fm = getSupportFragmentManager();
        if (fm != null) {
            // 遍历所有Fragment，找到需要更新的Fragment
            List<androidx.fragment.app.Fragment> fragments = fm.getFragments();
            for (androidx.fragment.app.Fragment fragment : fragments) {
                if (fragment instanceof UHFSetFragment) {
                    ((UHFSetFragment) fragment).onSecretFeatureEnabled();
                } else if (fragment instanceof UHFReadTagFragment) {
                    ((UHFReadTagFragment) fragment).onSecretFeatureEnabled();
                }
            }
        }
    }
